...
while True:
 ...
 # Control servo_4 based on finger gestures
        ...
        for box in boxes:
            ...
            if confidence > 50:
                ...
                for info in results:
                    ...
                    if conf > 80 and model_person.names[class_detect] == 'person':
                        cvzone.cornerRect(houseAI, [x1, y1, width, height], l=30, rt=6)
                        cvzone.putTextRect(houseAI, f'{model_person.names[class_detect]}', [x1 + 8, y1 - 12], thickness=2, scale=2)

                    if threshold < 0:
                        cvzone.putTextRect(houseAI, 'Fall Detected', [height, width], thickness=2, scale=2)

    result_person = model_person(houseAI_resize, stream=True)
